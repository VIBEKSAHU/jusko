...
async function getAllApplication({ token }) {
  ...
  if (!usr) {
    ...
    // console.log("This is line of code is working for HOD");
    const applicants = await db.customer.findMany({
      where: {
        status: 3,
      },
    ...
    console.log("Application List : " + applicants);
    return {
      flag: true,
      message: "Success",
    ...
    // console.log("This is line of code is working fors Accounts");
  ...
  } else if (usr.user_role == 7) {
    const applicants = await db.customer.findMany({
      //field worker's dashboard
      where: {
        status: 1,
      },
      orderBy: [
