...
export default function FieldWorkerDashboard(props) {
  ...
  const [Table, setTable] = useState(
    <ApplicationTable
      data={applicants}
      actionLink={props.link || "/fieldWorkerApplicationDetails"}
    />
  );
  ...
  const fetchApi = async (check) => {
    ...
    console.log(check == 0);
    if (check == 0 || check == 13) {
      setLink(props.link || "/fieldWorkerApplicationDetails")
      fetchApplicants();
    ...
    <div>
      <Container maxWidth="xl" sx={styles.container}>
        ...
        <Typography sx={styles.dashboardText}>
          {props.admin || "Field Staff"} Dashboard
        </Typography>
        <p style={{ alignSelf: "flex-end", marginTop: "-40px", fontWeight: "bolder" }}>{props.userName} | Field Worker</p>
            ...
            {(popupState) => (
              <React.Fragment>
                ...
                <Menu {...bindMenu(popupState)}>
                  ...
                  <MenuItem
                    id="rejectedByCustomer"
                    onClick={(e) => {
                      setCheck(11);
                      setTableName("rejected by customer");
                      localStorage.setItem("statusOfApplication", "rejected by customer")

                      console.log("fetching new Pending rejected by customer");
                      popupState.close();
                      // handleClickPop(e);
                    }}
                  >
                    Application Rejected By Customer
                  </MenuItem>
                  <MenuItem
                    id="under_l2_wr"
                    onClick={(e) => {
